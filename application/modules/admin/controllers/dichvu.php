...
class dichvu extends MY_Controller{
	...
	public function __construct(){
        ...
        $this->_gallery_url = base_url()."public/default/dichvu/";
        //Lấy đường dẫn vật lý của thư mục chứa hình ảnh đươc upload
        $this->_gallery_path = realpath(APPPATH. "../public/default/dichvu");
	}
	public function listall(){
		...
		$config['base_url'] = base_url().'admin/dichvu/listall?';
		//config phân trang
		$config['total_rows'] = $this->mdichvu->numRows();
		$config['per_page'] =15;
		...
		$this->_data['template'] = 'admin/dichvu/list_view';
		$this->_data['title'] = 'Trang Quản Lý Danh Mục ';
		$this->_data['info'] = $this->mdichvu->listAll($config['per_page'],$start);
	...
	public function add(){
		...
		if(isset($_POST['ok'])){
			...
			if($this->form_validation->run()==TRUE){
				...
				$this->mdichvu->add($arr);
				$this->session->set_flashdata('ses_flash',"Thêm");
				redirect(base_url()."admin/dichvu/listall");
			}
            $arr = array(
                'title' => $this->input->post('title'),
				'link' => $this->input->post('link'),
		...
		$this->_data['template'] = 'admin/dichvu/modify_view';
	...
	public function edit($id){
		...
		if(isset($_POST['ok'])){
            ...
            if($this->form_validation->run()==TRUE){
				...
				$this->mdichvu->editById($id,$arr);
				$this->session->set_flashdata('ses_flash',"Sửa");
				redirect(base_url()."admin/dichvu/listall");
			}
		}
		$this->_data['info'] = $this->mdichvu->getById($id);
		$this->_data['template'] = 'admin/dichvu/modify_view';
