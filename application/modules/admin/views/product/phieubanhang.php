...
<head>
    ...
    <style>
        ...
        #customer input{width: auto!important;}
        @media print {
...
<div id="body">
    <div id="header">
        <?php $usr = $this->session->userdata('user'); if($usr['user_type']=='nhanvien'){ ?>
        <a href="<?php echo base_url() ?>admin/login/logout" class="print-hide" style="padding: 5px 20px; color: #FFF; background: #2573C3; border:none; cursor: pointer; font-family: Arial; font-size: 12px; position: absolute; right:10px">Đăng xuất</a>
        <?php } ?>
        <div id="logo">
    ...
    <div id="detail">
        <table border="1">
            ...
            <tr>
                <td colspan="6" style="text-align: right"><input class="print-hide" type="text" id="magiamgia" placeholder="Nhập mã giảm giá.." style="border: 1px solid #333; height: 20px; padding: 0 5px; float: left; display: block;width: 150px; margin-top: 5px; font-family: Arial; font-size: 12px;" /><button style="padding: 0 15px; height: 20px; margin-top: 5px; background: #2573C3; color: #fff; border:none; float:left; margin-left: 20px; font-family: Arial;" type="button" id="check_giamgia" class="print-hide">OK</button>GIẢM GIÁ</td>
    ...
    <tr id="clone">
        ...
        <td style="white-space: nowrap">
            ...
            <select  style="min-width: 250px" data-placeholder="Chọn Sản Phẩm" name="product" class="product">
                ...
                <?php foreach($product as $val){ ?>
                    <option value="<?php echo $val['id'] ?>"><?php echo $val['name'].' - '.$val['barcode']?></option>
                <?php } ?>
            </select>
        </td>
        <td>
        ...
        <td>
            <select class="vat runable">
                ...
                <option value="0" selected="selected">Không Xuất Hóa Đơn</option>
            </select>
        </td>
...
$(document).ready(function(){
    ...
    $("#detail").on('change','.product',function(){
        ...
        $.ajax({
            ...
            success: function(string){
                ...
                seft.parent().parent().find('.soluong').val('1');
    ...
    $("#detail").on('click','.remove', function(){
        ...
        var tong = 0;
        var tong_vat = 0;
        var giamgia = $('#giamgia').html().split(".").join("");
        ...
        $("#total").html((tong+tong_vat-parseInt(giamgia)).formatMoney(0,',','.'));
        // chuyen sang chu
        var url = $("#base_url").val();
        var page_module = $('#page_module').val();

        $.post("<?php echo base_url() ?>"+"admin/product/convert", { tong : tong + tong_vat - parseInt(giamgia) }).done(function(data){
            $("#text_total").val(data)
        });
...
//--------------------------------------------
    ...
    $("#detail").on('change','.runable', function(){
        ...
        $("#vat").html((tong_vat).formatMoney(0,',','.'));
        $("#total").html((tong+tong_vat-parseInt(giamgia)).formatMoney(0,',','.'));
        $('#check_giamgia').trigger('click');
        setTimeout(function(){
            var tong = 0;
            var tong_vat = 0;
            giamgia = $('#giamgia').html().split(".").join("");
            $('.sotien').each(function(){
                var monney = $(this).val().split(".").join("");
                var vat =$(this).parent().parent().find('.vat').val();
                tong = tong + parseInt(monney);
...
//-------
    $(document).on('submit','#form',function(){
        ...
        var seller = '<?php $user = $this->session->userdata('user'); echo $user['user_id']; ?>';
        
        $.ajax({
            type: "POST",
            url: "<?php echo base_url() ?>"+'admin/product/save',
            data: $('#form').serialize()+'&detail=' + JSON.stringify(detail)+'&total='+total+'&vat='+vat+'&mapt='+code+'&giamgia='+giamgia+'&magiamgia='+magiamgia+'&seller='+seller
        }).done(function(a){
                //alert(a);
    ...
    $('#check_giamgia').click(function(){
        ...
        var total = tong+tong_vat;
        $.post('<?php echo base_url() ?>default/index/check_voucher2',{magiamgia:magiamgia,total:total},function(data){
            if(data!='0'){
                total = total - parseInt(data);
                $('#giamgia').html(parseInt(data).formatMoney(0,',','.'));
                $("#total").html(total.formatMoney(0,',','.'));  
                setTimeout(function(){
                    var tong = 0;
                    var tong_vat = 0;
                    giamgia = $('#giamgia').html().split(".").join("");
                    ...
                    $.post("<?php echo base_url() ?>"+"admin/product/convert", { tong : tong + tong_vat - parseInt(data) }).done(function(data){
                        $("#text_total").val(data)
                    });
                },1000) 
            }
